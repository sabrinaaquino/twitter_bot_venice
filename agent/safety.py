...
⚠️ DUPLICATE OF the top-level ``safety.py`` (copied so the agent owns its full
security stack and the legacy bot + legacy ``safety.py`` can be retired later).
Until the legacy bot is gone, **keep this file in sync with the top-level
``safety.py``** — a fix to one must be mirrored to the other. The agent imports
from ``agent.safety`` exclusively; the legacy path imports the top-level one.
